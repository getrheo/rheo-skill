...
```text
stack, text, image, lottie, video, icon, button, back_button, progress, loader, counter, single_choice, multiple_choice, text_input, scale_input, wheel_picker, oauth_provider, oauth_login, email_password_auth, email_password_field, email_password_submit, carousel, hyperlink, checkbox, conditional
```
...
- Use at most one input layer kind per **active path** (`single_choice`, `multiple_choice`, `text_input`, `scale_input`, `wheel_picker`). Without a `conditional` layer that means one per screen; sibling `conditional` branches may each own their own input because only one branch ever renders.
...
- **Integration Node** — partner providers such as `revenuecat` (requires App settings toggle).
...
- Screens with `text_input`, `multiple_choice`, `scale_input`, or `wheel_picker` include a `button` with `action.kind: "continue"`.
...
- External surfaces: real `config.provider` (`revenuecat` for Integration Nodes or `headless` for External Surface Nodes), connected `fallback`, RevenueCat enabled when that provider is used.
